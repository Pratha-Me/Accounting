...
<script>
	...
	function edit(sn) {
		var id = ['id', 'name', 'startDate', 'endDate', 'status'];
	...
	function doUpdate()
	{
		...
		var URL = "<%=path%>/api/Fiscal/" + document.getElementById('id').value;
	...
	function getRecord()
	{
		...
		$.ajax({type: "GET", url: URL, headers: {'Authorization': '<%=token%>'}, contentType: "application/json; charset=utf-8", dataType: "json", success: function (data) {
				...
				document.getElementById('table').innerHTML = "<table class = 'responstable' id='dataTable'><tr> <th hidden> <span> id </span> </th><th><span>Fiscal Name</span> </th> <th> <span> Start Date</span> </th><th> <span> End Date</span> </th><th> <span> Status</span> </th><th><span> Action</span> </th></tr></table>";
				var tableData;
				for (var i = 0; i < data.length; i++)
				{
					tableData = "<tr><td hidden>" + data[i].id + "</td><td>" + data[i].name + "</td><td>" + data[i].startDate + "</td><td>" + data[i].endDate + "</td><td>" + data[i].status + "</td><td><a onclick='edit(" + (i + 1) + ")'><i class=\"fas fa-edit\"></i></a> | <a onclick='recordDelete(" + data[i].id + ")' ><i class=\"fas fa-trash-alt\"></i></a></td></tr>";
