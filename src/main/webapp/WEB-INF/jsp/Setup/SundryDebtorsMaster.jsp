...
<input type="text" id="sundryDebtorsName" onkeyup="getSundryDebtors('sundryDebtorsName','sundryDebtors')" onkeydown="moveFocus(event,'sundryDebtorsName','sundryDebtors');" onfocus=" showAc('sundryDebtorsName','sundryDebtors') " class="form-control">
...
var id = ['id','code','name','address','panVatNo','status','sundryDebtors','mobileNo'];
...
var URL="<%=path%>/api/Setup/SundryDebtorsMaster";
callApi(URL,requestData,"POST");
}
 function doUpdate()
{
  var dataForm = $('form').serializeArray(); var requestData = {}; $.each(dataForm, function(i, v) {requestData[v.name] = v.value;});
 var URL="<%=path%>/api/Setup/SundryDebtorsMaster/"+document.getElementById('id').value;
...
var URL="<%=path%>/api/Setup/SundryDebtorsMaster/"+id;
callApi(URL,"","DELETE");
}
 getRecord();
</script>
